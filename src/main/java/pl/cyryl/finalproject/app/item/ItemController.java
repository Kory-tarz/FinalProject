...
public class ItemController {
    ...
    public String createNewItem(Model model, @Valid Item item, BindingResult result, @RequestParam("images") MultipartFile[] multipartFiles) {
        if (result.hasErrors()) {
            setEditItemAttributes(model, item);
            return "item/add";
        }
        try {
    ...
    public String editItem(HttpSession session, Model model, @PathVariable long id) {
        ...
        if (userId != item.getOwner().getId()) {
            throw new UnauthorizedAccessException("Attempt to modify data by user with id: " + userId);
        }
        setEditItemAttributes(model, item);
        return "item/edit";
    }
    ...
    public String search(Model model,
        ...
        Page<Item> items;
        try {
            items = itemService.findAllActive(pageNr, itemsPerPage, categoryId, sortColumnName, asc);
        } catch (PropertyReferenceException | IllegalArgumentException exception) {
            items = itemService.findAllActive(0, 10);
        }
        model.addAttribute(ITEM_LIST, items);
        model.addAttribute(CATEGORIES, categoryRepository.findAll());
        model.addAttribute("dirName", itemPhotoService.getDirectory());
    ...
    private void setEditItemAttributes(Model model, Item item) {
        model.addAttribute(ITEM_ATTRIBUTE, item);
        model.addAttribute(CATEGORIES, categoryRepository.findAll());
        model.addAttribute("dirName", itemPhotoService.getDirectory());
    }
